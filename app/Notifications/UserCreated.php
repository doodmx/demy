...
class UserCreated extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        ...
        return (new MailMessage)
            ->bcc(['contacto@demy.mx', 'gpichardo@azell.co'])
            ->success()
            ->greeting('¡¡ Hola ' . $notifiable->profile->lastname . ' ' . $notifiable->profile->name . '!!')
            ->subject(env('APP_NAME') . ' - Nuevo Usuario')
            ->line('Te damos la bienvenida a '.env('APP_NAME').' Esperamos disfrutes de tu capacitación.')

    ...
    <div class="card-header d-flex justify-content-between align-items-center" role="tab"
        ...
        <div>
            ...
            @if($chapter->nodes->count()>0)
                ...
                <a class="btn-floating btn-sm btn-secondary-two align-items-start mr-4"
        ...
        <!--Title-->
        <div class="btn-edit-chapter cursor-pointer {{!empty($chapter->video_link)?'chapter-link':''}}">
            @if(app()->getLocale() === 'es')
            ...
            @if($chapter->nodes->where('depth','>',0)->count() > 0)
                <h5>
                    <div class="badge badge-secondary">
                        <span class="count"
                              id="count{{$chapter->id}}">{{$chapter->nodes->where('depth','>',0)->count()}}</span>
                        capítulos
                    </div>
                </h5>
            @endif
